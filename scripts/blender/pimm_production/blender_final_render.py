...
def _blender_render_script(
    ...
    return "\n".join((
        ...
        f"shutil.copyfile(scene.render.filepath, {str(output_root / f'{shot_id}--animation-start.png')!r})",
        f"shutil.copyfile(scene.render.filepath, {str(output_root / f'{shot_id}--animation-end.png')!r})",
        "scene.render.image_settings.file_format = 'OPEN_EXR'",
        "scene.render.image_settings.color_depth = '32'",
        f"bpy.data.images['Render Result'].save_render(filepath={str(output_root / f'{shot_id}--transparent.exr')!r}, scene=scene)",
